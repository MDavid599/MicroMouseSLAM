...
bool CLinearSLAMImp::run( int argc, char** argv )
{
	bool bTrue = lmj_parseArgs( argc, argv );

	if ( !bTrue )
		return false;

	run( m_szSt, m_szPath, m_nMapCount, m_data, m_method, m_szInfo, m_szPose, m_szFea );
	return true;
}

bool CLinearSLAMImp::lmj_parseArgs( int argc, char* argv[] )
{
	int i;
	string param;
	int  nFlag[5];
	nFlag[0] = nFlag[1] = nFlag[2] = nFlag[3] = nFlag[4] = 0;
	...
	for (i = 1; i < argc; i++)
	{
		...
		if ( strcmp( name.c_str(), "path") == 0 )
		{
			i++;
			param = argv[i];
			m_szPath = (char*)malloc(param.length());
			strcpy( m_szPath, param.c_str() );
			nFlag[0] = 1;
		}

		if ( strcmp( name.c_str(), "st") == 0 )
		{
			i++;
			param = argv[i];
			m_szSt = (char*)malloc(param.length());
			strcpy( m_szSt, param.c_str() );
			nFlag[1] = 1;
		}
		...
		if ( strcmp( name.c_str(), "i") == 0 )
		{
			i++;
			param = argv[i];
			m_szInfo = (char*)malloc(param.length());
			strcpy( m_szInfo, param.c_str() );
			nFlag[1] = 1;
		}

		if ( strcmp( name.c_str(), "p") == 0 )
		{
			i++;
			param = argv[i];
			m_szPose = (char*)malloc(param.length());
			strcpy( m_szPose, param.c_str() );
		}

		if ( strcmp( name.c_str(), "f") == 0 )
		{
			i++;
			param = argv[i];
			m_szFea = (char*)malloc(param.length());
			strcpy( m_szFea, param.c_str() );
		}

		if ( strcmp( name.c_str(), "num") == 0 )
		{
			i++;
			param = argv[i];
			m_nMapCount = atoi( param.c_str());
			nFlag[2] = 1;
		}

		if ( strcmp( name.c_str(), "meth") == 0 )
		{
			i++;
			param = argv[i];
			if( strcmp(param.c_str(), "SEQ") == 0)
			{
				m_method = LinearSLAM_Sequential;
				nFlag[3] = 1;
			}

			if( strcmp(param.c_str(), "DC") == 0)
			{
				m_method = LinearSLAM_DivideConquer;
				nFlag[3] = 1;
			}
		}
		...
		if ( strcmp( name.c_str(), "type") == 0 )
		{
			i++;
			param = argv[i];
			if( strcmp(param.c_str(), "2DPF") == 0)
			{
				m_data = LinearSLAM_2DPoseFeature;
				nFlag[4] = 1;
			}

			if( strcmp(param.c_str(), "3DPF") == 0)
			{
				m_data = LinearSLAM_3DPoseFeature;
				nFlag[4] = 1;
			}

			if( strcmp(param.c_str(), "2DPG") == 0)
			{
				m_data = LinearSLAM_2DPoseGraph;
				nFlag[4] = 1;
			}

			if( strcmp(param.c_str(), "3DPG") == 0)
			{
				m_data = LinearSLAM_3DPoseGraph;
				nFlag[4] = 1;
			}
		}
	}
	...
	if ( !nFlag[0] )
	{	printf( "LinerSLAM Error: Please Input Right File Path:\n" );	return false;	}


	if ( !nFlag[2] )
	{	printf( "LinerSLAM Error: Please Set Local Map Number:\n" );	return false;	}


	if ( !nFlag[3] )
	{	printf( "LinerSLAM Error: Please Set Process Method:\n" );	return false;	}


	if ( !nFlag[4] )
	{	printf( "LinerSLAM Error: Please Set Data Type:\n" );	return false;	}

	return true;

}

void CLinearSLAMImp::lmj_printHelp()
{
	printf( "Linear SLAM Solution General Options\n" );
	printf( "\n" );

	printf( "-path			Set Data Path.\n" );
	printf( "-st            Set Path to Save Final State Vector\n");
	printf( "-i			Set Path to Save Infomation Matrix\n");
	printf( "-p			Set Path to Save Poses\n");
	printf( "-f			Set Path to Save Features\n");
	printf( "-num			Local Map Number\n");
	printf( "-meth			Set Process Method\n" );
	printf( "Process Method Listed As Following:\n" );
	printf( "			I : SEQ(Sequential)\n" );
	printf( "			II: DC(DivideConquer)\n" );
	printf( "-type			Set Data Type.\n" );
	printf( "Data Type Listed As Following:\n" );
	printf( "			I  : 2DPF(2D Pose Feature)\n" );
	printf( "			II : 3DPF(3D Pose Feature)\n" );
	printf( "			III: 2DPG(2D Pose Graph)\n" );
	printf( "			IV : 3DPG(3D Pose Graph)\n" );
