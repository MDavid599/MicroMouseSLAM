...
% 
%%
function [GMap,LM_L] = LMJ_DSLAM2D_Divide_Conquer(LM,LocalMapNum)
    ...
    for i=1:LocalMapNum;
        ...
        for j=1:NumLM;                        
            ...
            if j==1;
                ...
                [GMap_End] = FuncTransform_DSLAM2D(GMap,LM_L{L}{2*(i-1)+j}.Ref);
                [GMap] = FuncLinearLS_DSLAM2D(GMap_End,LM_L{L}{2*(i-1)+j});
            end;
        end;
        
        if rem(i,2) == 0;
            [GMap] = FuncTransform_DSLAM2D(GMap,PreRef);
        end;
        GMap.PreRef = PreRef;
